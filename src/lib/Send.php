...
class Send {
  ...
  private function _returnSend($returnMailTitle, $returnMailHeader, $returnMailFooter) {
    ...
    // タイトルの設定
    $returnTitle = "{$returnMailTitle}";
    $returnTitle = mb_encode_mimeheader($returnTitle, "ISO-2022-JP-MS","UTF-8");

    // メッセージの設定
    $this->returnMessage = "{$this->_requiredItem["name"]} 様\n";
    $this->returnMessage .= "この度は、【Pono】へお問合せ頂きありがとうございます。\n";
    $this->returnMessage .= "下記内容で承りましたので、担当よりご連絡差し上げます。\n\n";
    $this->returnMessage .= "※お問い合わせが18時以降の場合は、翌営業日のご連絡になります。ご了承下さい。\n";
    $this->returnMessage .= "※本メールは、プログラムから自動で送信しています。\n";
    $this->returnMessage .= "-------------------------------------------------\n\n";
    foreach ($this->_submitContent as $key => $value) {
